const useCases = [
  {
    ...
    conditions: "COND_A_DEPOSITED | COND_B_DEPOSITED",
    bitmask: "= 3",
    icon: (
  ...
  },
  {
    ...
    conditions: "COND_A_DEPOSITED | COND_A_APPROVED",
    bitmask: "= 5",
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    conditions: "COND_A_DEPOSITED | COND_TIMELOCK",
    bitmask: "= 17",
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    conditions: "COND_A_DEPOSITED | COND_A_APPROVED | COND_B_APPROVED",
    bitmask: "= 13",
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    title: "Mutual Sign-Off",
    description:
      "Both parties must approve for settlement. Ideal for milestone-based agreements, partnership deals, and joint ventures.",
    conditions: "COND_A_APPROVED | COND_B_APPROVED",
    bitmask: "= 12",
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M10.125 2.25h-4.5c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125v-9M10.125 2.25h.375a9 9 0 019 9v.375M10.125 2.25A3.375 3.375 0 0113.5 5.625v1.5c0 .621.504 1.125 1.125 1.125h1.5a3.375 3.375 0 012.625 1.25" />
      </svg>
    ),
  },
  {
    title: "Arbiter-Only Resolution",
    description:
      "No auto-settle. A trusted arbiter must resolve. Perfect for complex disputes, escrow services, and insurance claims.",
    conditions: "release_conditions = 0",
    bitmask: "(manual)",
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    title: "Real-World Asset Settlement",
    description:
    ...
    conditions: "COND_A_DEPOSITED | COND_B_DEPOSITED | COND_A_APPROVED",
    bitmask: "= 7",
    icon: (
      <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function UseCases() {
  return (
    <section id="use-cases" className="relative py-16 sm:py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          ...
          <p className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight">
            One primitive,{" "}
            <span className="gradient-text">infinite agreements</span>
          </p>
          <p className="mt-4 text-text-muted text-sm sm:text-lg">
            The bitmask condition system lets you configure any settlement logic
            from a single Agreement struct.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {useCases.map((uc, i) => (
            <div
              key={i}
              className="glow-card group rounded-2xl border border-border/50 bg-surface/50 p-6 sm:p-8"
            >
              <div className="text-accent mb-4 sm:mb-5 group-hover:text-accent-secondary transition-colors">
                {uc.icon}
              </div>
              <h3 className="text-base sm:text-lg font-bold mb-2">{uc.title}</h3>
              <p className="text-text-muted text-xs sm:text-sm leading-relaxed mb-4">
                {uc.description}
              </p>
              <div className="font-mono text-[10px] sm:text-xs text-accent/70 bg-accent/5 border border-accent/20 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 break-all">
                <span className="text-text-muted">conditions:</span> {uc.conditions}
                {uc.bitmask && (
                  <>
                    {" "}
                    <span className="text-accent">{uc.bitmask}</span>
                  </>
                )}
              </div>
            </div>
